...
# Загружаем шрифты для выводимых надписей
...
bat_move = 0
# Cоздание "движения" крыльев засчет смены изображения каждые 40 мс
BATEVENT = pygame.USEREVENT + 1
pygame.time.set_timer(BATEVENT, 40)
bat_counter = 0
...
def menu_design(font_menu_1, font_menu_2):
    ...
    :param font_menu_1: pygame.font.Font: шрифт для меню №1
    :param font_menu_2: pygame.font.Font: шрифт для меню №2
    """
...
def count_points(pipes, score):
    ...
    :param pipes: list: список колонн
    :param score: int: текущие очки
    :return: int: текущие очки + 1 за каждую пройденную колонну
    """
    try:
        for pipe in pipes:
...
def score_display(if_game, score, record_score, font):
    ...
    :param if_game: bool: параметр отвечающий за статус игры: True - игра активна, False - меню игры
    :param score: int: текущие очки
    :param record_score: int: максимальное количество набранных очков
    :param font: pygame.font.Font: шрифт для отображения очков
    """
...
def check_Collision(pipes, bat_rect):
    """ Функция, проверяющая столкновение хитбокса мыши с колонной и его выход за пределы экрана
    :param pipes: list: список колонн
    :param bat_rect: pygame.Rect: положение мыши на экране
    :return: bool: True - в случае выхода за пределы экрана или столкновения с колонной, иначе - False
    """
...
def create_clouds(clouds, clouds_x):
    """ Функция вывода на экран изображения облаков и добавления справа вслед за текущим такого же изображения
    :param clouds: pygame.Surface: изображение облаков
    :param clouds_x: int: параметр смещения облаков по Х
    """
    try:
...
def create_pipe():
    ...
    :return: pygame.Surface: Колонна в нижней (pipe_normal) и колонна в верхней (pipe_flip) частях экрана
    """
...
def move_pipes(pipes):
    ...
    :param pipes: list: список колонн
    :return: list: Обновленный список колонн со смещением по Х
    """
    try:
        for pipe in pipes:
            pipe.centerx -= 4
